"""BTCUSDT liquidity heatmap dashboard (version 13).

This script fetches order book data from Binance using ccxt, stores snapshots
in Redis, and displays a heatmap with candlestick overlay and volume profile.
The timeframe can be selected from a dropdown.
"""
...
PRICE_STEP = 5.0  # USDT rounding step for heatmap levels
...
def load_history() -> List[Dict[str, Any]]:
    """Load all snapshots stored in Redis."""
...
def build_heatmap(history: List[Dict[str, Any]]):
    """Return price levels, times, and heatmap matrix."""
    if not history:
        return [], [], np.array([[]])
...
def build_candles(history: List[Dict[str, Any]], timeframe: str) -> pd.DataFrame:
    """Aggregate mid prices from history into OHLC candles."""
    if not history:
...
def make_figure(tf: str) -> go.Figure:
    """Create Plotly figure with heatmap, candles, and volume profile."""
